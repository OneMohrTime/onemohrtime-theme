	...
	function ajax_render_location( $options = array() )
	{
		...
		switch($options['param'])
		{
			...
			case "post_status" :
				...
				$choices = array(
					'publish'	=> __( 'Publish', 'maca' ),
					'pending'	=> __( 'Pending Review', 'maca' ),
					'draft'		=> __( 'Draft', 'maca' ),
					'future'	=> __( 'Future', 'maca' ),
					'private'	=> __( 'Private', 'maca' ),
					'inherit'	=> __( 'Revision', 'maca' ),
					'trash'		=> __( 'Trash', 'maca' )
				);
			...
			case "user_type" :
				...
				if( is_multisite() )
				{
					$choices['super_admin'] = __('Super Admin', 'maca');
				}
